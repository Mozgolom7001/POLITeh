...
Функция СобратьДанныеПоРеализацияТоваровУслуг(ДокОснование)
	...
	//ВЦ+
	Если Шапка.ОтпускРазрешил <> NULL Тогда
		//смена 17.09.2018
		//ОтпускРазрешил = ОбщегоНазначения.ФамилияИнициалыФизЛица(СокрЛП(Шапка.ОтпускРазрешил)) + " по приказу №1 от 09.01.18";
		ОтпускРазрешил = СокрЛП(Шапка.ОтпускРазрешил) + " по приказу №1 от 09.01.18";
		ДанныеДляПечати.Вставить(?(ЭтоПБОЮЛ(Шапка.Поставщик), "ФИОПБОЮЛ", "ФИОРуководителя"), ОтпускРазрешил);
		ДанныеДляПечати.Вставить("ФИОГлавногоБухгалтера", ОтпускРазрешил);
	Иначе
		ДанныеДляПечати.Вставить(?(ЭтоПБОЮЛ(Шапка.Поставщик), "ФИОПБОЮЛ", "ФИОРуководителя"), Руководители.Руководитель);
		ДанныеДляПечати.Вставить("ФИОГлавногоБухгалтера", Руководители.ГлавныйБухгалтер);
	КонецЕсли; 
